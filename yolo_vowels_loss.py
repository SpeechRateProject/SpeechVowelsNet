...
class YoloVowelsLoss:
    ...
    def loss(self, y_pred, counter_output, y_true, config_dict, use_cuda=False):
        """
        ...
        # if want to check what is the maximum number of vowels in same cell
        # if torch.max(total_per_cell_y_true) > 1.0 :
        #     print(f"max k-words in cell: {torch.max(total_per_cell_y_true)}")
        # _, pred = torch.max(counter_output, 1)
        loss_function = nn.CrossEntropyLoss()
        total_y_true = total_y_true.to(dtype=torch.int64)

        six_part = loss_function(counter_output, total_y_true)

        total_loss = torch.mean(first_part) + torch.mean(second_part) + torch.mean(third_part) \
                     + torch.mean(fourth_part) + torch.mean(fifth_part) + six_part

        return total_loss, torch.mean(first_part), torch.mean(second_part), torch.mean(third_part), \
               torch.mean(fourth_part), torch.mean(fifth_part), six_part
