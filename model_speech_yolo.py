...
class SpeechYoloVowelNet(nn.Module):
    ...
    def __init__(self, classfication_model, c=7, b=2, k=10, dropout=0):
        ...
        """
        super(SpeechYoloVowelNet, self).__init__()
        self.c = c
        self.b = b
        self.k = k
        ...
        #counter is the max number of vowels in a file to predct - can be between 0 to 10
        self.counter = 10
    ...
    def forward(self, x):
        ...
        return final_output, count_prediction
...
def load_model(save_dir):
    ...
    if arc_type.startswith('VGG'):
        model_ = VGG(arc_type)
        speech_net = SpeechYoloVowelNet(model_, config_dict["C"], config_dict["B"], config_dict["K"])
    ...
    ratio = 0
    if 'correct_ratio' in checkpoint.keys():
        ratio = checkpoint['correct_ratio']

    no_object_object_wrong_ratio = np.inf
    if 'no_object_object_wrong_ratio' in checkpoint.keys():
        no_object_object_wrong_ratio = checkpoint['no_object_object_wrong_ratio']

    return speech_net, checkpoint['acc'], checkpoint['epoch'], ratio, no_object_object_wrong_ratio
...
def create_speech_model(pretrained_model, arc, config_dict, dropout):
    if pretrained_model:
        if arc.startswith('VGG'):
            ...
            speech_net = SpeechYoloVowelNet(model_, config_dict["C"], config_dict["B"], config_dict["K"], dropout=dropout)
        else:
            raise Exception("No such architecture")
    else:
        if arc.startswith('VGG'):
